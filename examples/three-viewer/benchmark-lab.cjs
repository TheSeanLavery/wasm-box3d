...
function summarize(result) {
  ...
  return {
    ...
    maxDynamicTreeRebuildMs: Math.max(0, ...samples.map((sample) => sample.dynamicTreeRebuildMs ?? 0)),
    maxDynamicTreeRebuildLeaves: Math.max(0, ...samples.map((sample) => sample.dynamicTreeRebuildLeaves ?? 0)),
    maxBroadphasePairsMs: Math.max(0, ...samples.map((sample) => sample.broadphasePairsMs ?? 0)),
    maxBroadphaseNodes: Math.max(0, ...samples.map((sample) => sample.broadphaseNodes ?? 0)),
    maxBroadphaseCreated: Math.max(0, ...samples.map((sample) => sample.broadphaseCreated ?? 0)),
    maxCollideMs: Math.max(0, ...samples.map((sample) => sample.collideMs ?? 0)),
    maxCollideTaskMs: Math.max(0, ...samples.map((sample) => sample.collideTaskMs ?? 0)),
    maxCollideContactStateMs: Math.max(0, ...samples.map((sample) => sample.collideContactStateMs ?? 0)),
    maxCollideTotalContacts: Math.max(0, ...samples.map((sample) => sample.collideTotalContacts ?? 0)),
    maxCollideRecycledContacts: Math.max(0, ...samples.map((sample) => sample.collideRecycledContacts ?? 0)),
    maxCollideUpdatedContacts: Math.max(0, ...samples.map((sample) => sample.collideUpdatedContacts ?? 0)),
    maxCollideDisjointContacts: Math.max(0, ...samples.map((sample) => sample.collideDisjointContacts ?? 0)),
    maxCollideStartedTouching: Math.max(0, ...samples.map((sample) => sample.collideStartedTouching ?? 0)),
    maxCollideStoppedTouching: Math.max(0, ...samples.map((sample) => sample.collideStoppedTouching ?? 0)),
    maxCollideManifoldContacts: Math.max(0, ...samples.map((sample) => sample.collideManifoldContacts ?? 0)),
    maxCollideSatCalls: Math.max(0, ...samples.map((sample) => sample.collideSatCalls ?? 0)),
    maxCollideSatCacheHits: Math.max(0, ...samples.map((sample) => sample.collideSatCacheHits ?? 0)),
    maxCollideSatSameHullCalls: Math.max(0, ...samples.map((sample) => sample.collideSatSameHullCalls ?? 0)),
    maxCollideSatBoxHullCalls: Math.max(0, ...samples.map((sample) => sample.collideSatBoxHullCalls ?? 0)),
    maxCollideSatCacheSeparationHits: Math.max(0, ...samples.map((sample) => sample.collideSatCacheSeparationHits ?? 0)),
    maxCollideSatCacheFaceHits: Math.max(0, ...samples.map((sample) => sample.collideSatCacheFaceHits ?? 0)),
    maxCollideSatCacheEdgeHits: Math.max(0, ...samples.map((sample) => sample.collideSatCacheEdgeHits ?? 0)),
    maxCollideSatFullSearches: Math.max(0, ...samples.map((sample) => sample.collideSatFullSearches ?? 0)),
    maxCollideRecycleCandidates: Math.max(0, ...samples.map((sample) => sample.collideRecycleCandidates ?? 0)),
    maxCollideRecycleMissingCache: Math.max(0, ...samples.map((sample) => sample.collideRecycleMissingCache ?? 0)),
    maxCollideRecycleFastMesh: Math.max(0, ...samples.map((sample) => sample.collideRecycleFastMesh ?? 0)),
    maxCollideRecycleTested: Math.max(0, ...samples.map((sample) => sample.collideRecycleTested ?? 0)),
    maxCollideRecycleRejectedAngular: Math.max(0, ...samples.map((sample) => sample.collideRecycleRejectedAngular ?? 0)),
    maxCollideRecycleRejectedLinear: Math.max(0, ...samples.map((sample) => sample.collideRecycleRejectedLinear ?? 0)),
    maxCollideRecycleRejectedArc: Math.max(0, ...samples.map((sample) => sample.collideRecycleRejectedArc ?? 0)),
    maxSolveMs: Math.max(0, ...samples.map((sample) => sample.solveMs ?? 0)),
    maxSolverActiveColors: Math.max(0, ...samples.map((sample) => sample.solverActiveColors ?? 0)),
    maxSolverWideContacts: Math.max(0, ...samples.map((sample) => sample.solverWideContacts ?? 0)),
    maxSolverOverflowContacts: Math.max(0, ...samples.map((sample) => sample.solverOverflowContacts ?? 0)),
    maxSolverManifolds: Math.max(0, ...samples.map((sample) => sample.solverManifolds ?? 0)),
    maxSolverGraphBlocks: Math.max(0, ...samples.map((sample) => sample.solverGraphBlocks ?? 0)),
    maxSolverConstraintsMs: Math.max(0, ...samples.map((sample) => sample.solverConstraintsMs ?? 0)),
    maxSolverPrepareMs: Math.max(0, ...samples.map((sample) => sample.solverPrepareMs ?? 0)),
    maxSolverPrepareJointsMs: Math.max(0, ...samples.map((sample) => sample.solverPrepareJointsMs ?? 0)),
    maxSolverPrepareWideContactsMs: Math.max(0, ...samples.map((sample) => sample.solverPrepareWideContactsMs ?? 0)),
    maxSolverPrepareMeshContactsMs: Math.max(0, ...samples.map((sample) => sample.solverPrepareMeshContactsMs ?? 0)),
    maxSolverPrepareOverflowMs: Math.max(0, ...samples.map((sample) => sample.solverPrepareOverflowMs ?? 0)),
    maxSolverSolveImpulsesMs: Math.max(0, ...samples.map((sample) => sample.solverSolveImpulsesMs ?? 0)),
    maxSolverRelaxImpulsesMs: Math.max(0, ...samples.map((sample) => sample.solverRelaxImpulsesMs ?? 0)),
