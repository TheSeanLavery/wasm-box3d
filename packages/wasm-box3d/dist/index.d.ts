...
export type Box3DDemo = {
  ...
  appendTrackSection(center?: Vec3Like, right?: Vec3Like, up?: Vec3Like, slot?: number): number;
  commitTrackMesh(slot?: number): number;
  activateTrackMesh(slot?: number): boolean;
  setTrackMeshEnabled(slot?: number, enabled?: boolean): boolean;
  ...
  setBodyLinearVelocity(bodyIndex: number, velocity?: Vec3Like): boolean;
  getBodyLinearVelocity(bodyIndex: number): { x: number; y: number; z: number };
