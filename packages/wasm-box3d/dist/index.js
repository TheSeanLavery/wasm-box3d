...
export async function createBox3DDemo(options = {}) {
  ...
  const api = {
    ...
    resetEmptyRaw: module.cwrap('wb3_reset_empty', 'number', []),
    beginTrackMeshRaw: module.cwrap('wb3_begin_track_mesh', 'number', Array(8).fill('number')),
    appendTrackSectionRaw: module.cwrap('wb3_append_track_section', 'number', Array(10).fill('number')),
    commitTrackMeshRaw: module.cwrap('wb3_commit_track_mesh', 'number', ['number']),
    activateTrackMeshRaw: module.cwrap('wb3_activate_track_mesh', 'number', ['number']),
    setTrackMeshEnabledRaw: module.cwrap('wb3_set_track_mesh_enabled', 'number', ['number', 'number']),
    ...
    spawnBoxQuatExRaw: module.cwrap('wb3_spawn_box_quat_ex', 'number', Array(19).fill('number')),
    spawnBoxQuatExNoSyncRaw: module.cwrap('wb3_spawn_box_quat_ex_no_sync', 'number', Array(19).fill('number')),
    ...
    setBodyLinearVelocityRaw: module.cwrap('wb3_set_body_linear_velocity', 'number', [
      'number',
      'number',
      'number',
      'number',
    ]),
    getBodyLinearVelocityComponentRaw: module.cwrap('wb3_get_body_linear_velocity_component', 'number', [
      'number',
      'number',
    ]),
  ...
  return {
    ...
    resetEmpty() {
      return api.resetEmptyRaw();
    },
    beginTrackMesh(options = {}) {
      const color = options.color ?? {};
      return api.beginTrackMeshRaw(
        options.slot ?? 0,
        options.halfWidth ?? 1.6,
        options.railHeight ?? 0.82,
        options.railWidth ?? 0.24,
        options.friction ?? 0.68,
        color.r ?? color.x ?? 0.08,
        color.g ?? color.y ?? 0.2,
        color.b ?? color.z ?? 0.31
      ) === 1;
    },
    appendTrackSection(center = {}, right = {}, up = {}, slot = 0) {
      return api.appendTrackSectionRaw(
        slot,
        center.x ?? 0,
        center.y ?? 0,
        center.z ?? 0,
    ...
    commitTrackMesh(slot = 0) {
      return api.commitTrackMeshRaw(slot);
    },
    activateTrackMesh(slot = 0) {
      return api.activateTrackMeshRaw(slot) === 1;
    },
    setTrackMeshEnabled(slot = 0, enabled = true) {
      return api.setTrackMeshEnabledRaw(slot, enabled ? 1 : 0) === 1;
    },
    ...
    addBox(options = {}, addOptions = {}) {
      ...
      const color = options.color ?? {};
      const quaternion = options.quaternion ?? options.rotation;
      if (quaternion && Number.isFinite(quaternion.w)) {
        const spawnRaw = addOptions.sync === false ? api.spawnBoxQuatExNoSyncRaw : api.spawnBoxQuatExRaw;
        return spawnRaw(
          position.x ?? 0,
          position.y ?? 6,
          position.z ?? 0,
    ...
    setBodyLinearVelocity(bodyIndex, velocity = {}) {
      return (
        api.setBodyLinearVelocityRaw(bodyIndex, velocity.x ?? 0, velocity.y ?? 0, velocity.z ?? 0) !== 0
      );
    },
    getBodyLinearVelocity(bodyIndex) {
      return {
        x: api.getBodyLinearVelocityComponentRaw(bodyIndex, 0),
        y: api.getBodyLinearVelocityComponentRaw(bodyIndex, 1),
        z: api.getBodyLinearVelocityComponentRaw(bodyIndex, 2),
      };
    },
